...
static const NotificationType _listenedNotificationType = NotificationType.UNREAD_NOTIFICATION_COUNT;
...
Tween(begin: 1.0, end: 1.2).chain(CurveTween(curve: Curves.easeInOut)),
),
child: IconButton(
...
Navigator.push(
context,
MaterialPageRoute(builder: (context) => NotificationListWidget()),
);
setState(() {
_unreadCount = 0; // Reset unread count when opening the notification list
});
},
),
),
if (_unreadCount > 0) // Show the badge only if there are unread notifications
Positioned(
right: 2,
top: 2,
child: Container(
